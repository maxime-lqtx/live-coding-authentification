...
export default class AuthController {
  ...
  static async login(req: Request, res: Response) {
    try {
      ...
      const generateToken = jwt.sign({ user_id: userIfExist[0] && userIfExist[0].id, user_email: userIfExist[0] && userIfExist[0]?.email, role: "user" }, process.env.SECRET_KEY || "dgjshdfguykdshgdfkjhgfjdsf0011231141.20231$$", {
        expiresIn: "30d"
      })
